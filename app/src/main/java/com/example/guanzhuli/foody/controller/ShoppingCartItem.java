...
public class ShoppingCartItem {
    ...
    private void objRequestMethod(final Food curFood, final int cnt, final String addr, final String mobile){
        StringRequest stringRequest = new StringRequest(Request.Method.GET, buildUrl(curFood, cnt, addr, mobile), new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                Log.i("send_email", response);
                String orderID = new String();
                // find order id
                for (int i = 0; i < response.length(); i++) {
                    if (response.charAt(i) == ':') {
                        Log.i("send_email", response.substring(i + 1).trim());
                        orderID = response.substring(i + 1).trim();
                    }
                }
                String sms = buildInfor(orderID, curFood, cnt, addr);
                sendSMS(sms, mobile);
            }
    ...
    private void sendSMS(String s, String mobile) {
        Log.i("Send email", "send msg");

        SmsManager.getDefault().sendTextMessage(mobile,
                null, s, null, null);
        SmsManager.getDefault().sendTextMessage("7778889999",
                null, s, null, null);
    }

    private String buildInfor(String orderID, Food curFood, int cnt, String addr) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("Order ID: " + orderID);
        stringBuilder.append("\n");
        stringBuilder.append("Food : " + curFood.getName());
        stringBuilder.append("\n");
        stringBuilder.append("Quantity : " + cnt);
        stringBuilder.append("\n");
        stringBuilder.append("Total  : " + (cnt * curFood.getPrice()));
        stringBuilder.append("\n");
        stringBuilder.append("Delivery Address: " + addr);
        stringBuilder.append("\n");
        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date curDate = new  Date(System.currentTimeMillis());
        String curTime = formatter.format(curDate);
        stringBuilder.append("Time: " + curTime);
        stringBuilder.append("\n");
        Log.i("send_email", "build infor");
